...
teamsdf <- read.csv("C:/Users/drewc/OneDrive/Desktop/Research Assistant/Team VORP  2/teamsdf.csv")

#gamestats <- data.frame(NULL)
for(g in 1:nrow(gamesdf)){
  id <- gamesdf$id[g]
  team <- gamesdf$team[g]
  ...
  if(team == gamesdf$`Home/Neutral`[g]){
    adder$opp_id <- paste0(id, "-", gamesdf$`Visitor/Neutral`[g])
  } else {
    adder$opp_id <- paste0(id, "-", gamesdf$`Home/Neutral`[g])
  }
  adder$season_id <- gamesdf$season_id[g]
  adder$win <- gamesdf$win[g]
  adder$year <- gamesdf$year[g]
  adder$adj_VORP <- as.numeric(adder$VORP) * as.numeric(adder$min)
  adder$avg_VORP <- mean(adder$adj_VORP)
  
  gamestats <- rbind(gamestats, adder)
  
  print(paste0(round((g/nrow(gamesdf)) * 100, 3),"%"))
}
gamestats$link <- paste0(gamestats$player,"-",gamestats$year)
